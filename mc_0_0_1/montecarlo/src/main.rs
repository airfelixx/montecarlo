...
fn update_portfolio_value(
    ...
    portfolio.initial_investment *= 1.0 + yearly_return;    // Update the portfolio value based on the yearly return
    portfolio.initial_investment += portfolio.monthly_contributions * 12.0; // Annual contribution
    portfolio.initial_investment     // Return the updated portfolio value
}
...
fn simulate_portfolio(
    ...
    for _ in 0..portfolio.years {             // Loop through each year of the simulation
        ...
        current_value = update_portfolio_value(portfolio, yearly_return); // Update the portfolio value with the simulated return
    }

    OneSimulationResult { final_value: current_value }   // Return the final value of the portfolio after the simulation
}

fn run_simulations(portfolio: &mut Portfolio) -> SimulationsArray {
...
fn run_monte_carlo_simulation(portfolio: &mut Portfolio) {
...
fn main() {
    ...
    let mut portfolio = Portfolio {
        ...
        expected_yearly_return: 0.07,         // Expected annual return (7%)
        monthly_contributions: 100.0,          // Monthly contributions to the portfolio
        volatility: 0.15,                      // Annual volatility (15%)
        years: 10,                             // Number of years to simulate
        goal: 20000.0,                         // Financial goal to be achieved
        num_simulations: 1000,                 // Number of Monte Carlo simulations to run
    };
    run_monte_carlo_simulation(&mut portfolio);  // Run the Monte Carlo simulation with the configured portfolio
    println!("Monte Carlo simulation completed.");  // Print a message indicating the completion of the simulation
    println!("Final portfolio value: {:.2}", portfolio.initial_investment);  // Print the final value of the portfolio after all simulations
